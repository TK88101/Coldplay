...
每天点一个按钮标记上班/休息/年假/加班/补休，自动同步到 iPhone 日历。会计年度制（4/1-3/31），自动计算剩余可休天数。
...
- **一键打卡** — 五个胶囊按钮：上班(蓝) / 加班(红) / 休息(绿) / 年假(紫) / 补打卡(橘)，垂直排列
- **加班记录** — 滚轮选择起始/结束时间，写入用户已有的"加班"日历，满8小时进位为1天
- **休息分类** — 点击休息弹出选择：「正常休息」或「還休（扣加班）」，补休从剩余可休天数中扣除
- **年假** — 独立于休息的年假记录，写入用户已有的"年假"日历全天事件
- **补打卡** — 忘记打卡？选择历史日期补录（支持上班/加班/休息/年假/补休），布局与主界面一致
- **日历同步** — 自动创建"考勤"专用日历；年假/加班写入用户已有日历；切换类型自动清除旧事件
- **年度统计** — 会计年度制（4/1-3/31），底部栏显示上班天数、加班天数、休息天数、剩余可休天数。每年10天年假+加班天数=总可休天数，点击查看历年统计
...
- **免费签名** — 7 天过期，可通过 SideStore 自动续签。数据不丢失
