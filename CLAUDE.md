...
**UI:** iOS 26 Liquid Glass design. `ContentView` is the main view with five capsule action buttons (work/overtime/rest/annual leave/backfill), status card, and stats bar (work days / overtime days / rest days / remaining leave). Rest button shows confirmation dialog: "正常休息" or "還休（扣加班）". Backfill sheet has five vertical buttons matching main interface order (work/overtime/rest/annual leave/compensatory rest), with overtime opening a time picker sub-sheet. `SettingsView` provides reminder toggle, language switching, and CSV export. `TodayView`, `CalendarView`, `StatsView` exist in the repo but are unused (legacy from earlier TabView design).
