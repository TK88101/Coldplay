...
| 项目 | 说明 |
|------|------|
| 触发条件 | 主界面或补打卡入口提交加班时，目标日期已存在加班记录 |
| 交互 | 弹出 confirmationDialog，三选一：「取代」（destructive） / 「累加」 / 「取消」 |
| 取代 | 清掉该日所有既有加班记录 + 同步清除该日「加班」日历的对应事件，再写入新记录 |
| 累加 | 保留既有记录，追加新的一笔（适用真有多段加班的场景） |
| 取消 | 放弃本次提交，状态保持不变 |
| 默认行为 | 同日无加班记录时直接写入，不弹对话框 |
...
| 加班双向同步 | 加班事件被从日历 app 删除后，Coldplay 下次启动或回前景时自动移除对应本地记录并提示 Toast。仅适用于存有 calendarEventID 的记录（v1.4 build 9 起新建的加班）；旧记录不参与 reconcile 以避免误删 |
...
struct OvertimeRecord: Codable, Identifiable {
    ...
    let endTime: Date
    let createdAt: Date
    var calendarEventID: String?   // 对应日历事件 ID，用于双向同步（v1.4 build 9 起记录）
}
...
| 1.4 (build 5-6) | 2026-04-12 | 每日 12:00 打卡提醒（UNCalendarNotificationTrigger）；Ralph 自动化框架；日历权限被拒时的 Settings redirect 引导 |
| 1.4 (build 7) | 2026-04-18 | 加班即视为上班（自动补/覆盖上班记录）；加班时数显示优化（<8h 显示 h，≥8h 显示天）；修复打加班后 12:00 提醒未取消的问题 |
| 1.4 (build 8) | 2026-04-22 | 同日重复加班确认对话框（取代/累加/取消）；「取代」同步清掉日历既有加班事件避免重复 |
| 1.4 (build 9) | 2026-04-22 | 加班双向同步：日历中删除加班事件后，app 下次启动或回前景时自动移除对应本地记录并 Toast 提示 |
